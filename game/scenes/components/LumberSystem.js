export default class LumberSystem {
    ...
    async workCycle(npc) {
        while (this.isWorking) {
            try {
                ...
                if (!canReach) {
                    console.log(
                        "Não foi possível alcançar a árvore, tentando outra...",
                    );
                    await this.waitFor(1000);
                    continue;
                }
    ...
    findNearestTree(npc) {
        ...
        for (const [key, value] of Object.entries(
            ...
            if (
                ...
                const distance =
                    Math.abs(npc.gridX - x) + Math.abs(npc.gridY - y);
                ...
                const canReachTree = adjacentPositions.some(
                    (pos) =>
    ...
    async moveToTree(npc, tree) {
        ...
        const currentDistance =
            Math.abs(npc.gridX - tree.gridX) + Math.abs(npc.gridY - tree.gridY);
    ...
    findPathToTree(npc, tree) {
        ...
        while (queue.length > 0) {
            ...
            if (
                Math.abs(current.x - tree.gridX) +
                    Math.abs(current.y - tree.gridY) ===
                1
            ) {
                return current.path;
            }
        ...
        const cutInterval = setInterval(() => {
            if (tree.sprite && tree.sprite.active) {
                const text = this.scene.add
                    .text(tree.sprite.x, tree.sprite.y - 10, "Toc", {
                        fontSize: "20px",
                        fill: "#fff",
                        stroke: "#000",
                        strokeThickness: 2,
